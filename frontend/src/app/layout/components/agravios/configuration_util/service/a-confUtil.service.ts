...
export class ConfigurationUtilService {
    ...
    getAll(pagination?: { page?: number, limit?: number }) {
        ...
        let url = `${this.api}/v1/configuration-utils?rol=${rol}`;
        if (pagination?.page) url += `&page=${pagination.page}`;
        if (pagination?.limit) url += `&limit=${pagination.limit}`;
        return this.http.get<AnswerQuery<ConfigurationUtil[]>>(url, { headers })
            .toPromise()
    ...
    getAllData() {
        const headers = this.commonService.getHeaders();
        const rol = this.commonService.getRol();
        const url = `${this.api}/v1/configuration-utils?rol=${rol}`;
        return this.http.get<AnswerQuery<ConfigurationUtil[]>>(url, { headers })
            .toPromise()
            .then(response => response.data); 
    }
    ...
    getByUuid(uuid: string) {
        const headers = this.commonService.getHeaders();
        const rol = this.commonService.getRol();
        const url = `${this.api}/v1/configuration-utils/${uuid}?rol=${rol}`;
        return this.http.get<AnswerQuery<ConfigurationUtil>>(url, { headers })
            .toPromise()
            .then(response => response.data);
    }

    async create(data: { name: string, basicCosteHour: number }): Promise<AnswerQueryResponse> {
    ...
    async update(uuid: string, data: { name: string, basicCosteHour: number }): Promise<AnswerQueryResponse> {
