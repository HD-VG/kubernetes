...
@Injectable()
export class ConfigurationTypeWorkService {
    ...
    getAll(pagination?: { page?: number, limit?: number }) {
        ...
        let url = `${this.api}/v1/configuration-type-work?rol=${rol}`;
        if (pagination?.page) url += `&page=${pagination.page}`;
        if (pagination?.limit) url += `&limit=${pagination.limit}`;
        return this.http.get<AnswerQuery<ConfigurationTypeWork[]>>(url, { headers })
            .toPromise()
    ...
    getAllData() {
        const headers = this.commonService.getHeaders();
        const rol = this.commonService.getRol();
        const url = `${this.api}/v1/configuration-type-work?rol=${rol}`;
        return this.http.get<AnswerQuery<ConfigurationTypeWork[]>>(url, { headers })
            .toPromise()
            .then(response => response.data); 
    }
    ...
    getByUuid(uuid: string) {
        const headers = this.commonService.getHeaders();
        const rol = this.commonService.getRol();
        const url = `${this.api}/v1/configuration-type-work/${uuid}?rol=${rol}`;
        return this.http.get<AnswerQuery<ConfigurationTypeWork>>(url, { headers })
            .toPromise()
            .then(response => response.data);
    }
    ...
    async create(data: { name: string }): Promise<AnswerQueryResponse> {
        ...
        try {
            ...
            console.error('Error al crear configuration type Work:', error);
    ...
    async update(uuid: string, data: { name: string }): Promise<AnswerQueryResponse> {
        ...
        try {
            ...
            console.error('Error al actualizar configuration type Work:', error);
    ...
    async delete(uuid: string): Promise<AnswerQueryResponse> {
        ...
        try {
            ...
            console.error('Error al eliminar configuration type Work:', error);
