...
import { CreateConfigurationCalculationDto } from 'src/presentation/dtos/cc_configuration_calculations/index.dto';
import { IConfigurationCalculationRepositoryToken } from '../tokens/configuration_calculation.tokens';

@Injectable()
export class CreateConfigurationCalculationUseCase {
  constructor(
  ...
  async execute(dto: CreateConfigurationCalculationDto, userId: number) {
    return await this.configurationCalculationRepository.create(dto, userId);
  }
}
